...
class DiffusionPipeline(nn.Module):
    def __init__(self, 
        ...
        with torch.no_grad():
            if latent_embedder is not None:
                latent_embedder.load_state_dict(torch.load(latent_embedder_checkpoint, weights_only=True))
    ...
    def denoise(self, x_t, steps=None, condition=None, use_ddim=True, **kwargs):
        ...
        for i, t in tqdm(enumerate(reversed(timesteps_array)),total=len(timesteps_array)):
        ...
        if self.latent_embedder is not None:
            ...
            pred_u = self.latent_embedder.u_filter(z1)
            pred_v = self.latent_embedder.v_filter(z2)
            pred_c = self.latent_embedder.c_filter(sampled_z)
